...
BOT_TOKEN = "8119496054:AAF7cqk38T7kEsuMba179WGKsqwV84rE1Bk"
YML_FILE_PATH = ".github/workflows/main.yml"
BINARY_FILE_NAME = "soul"
ADMIN_IDS = [2125948411] 
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if is_owner(user_id):
        if is_primary_owner(user_id):
            user_role = "👑 ᴘʀɪᴍᴀʀʏ ᴏᴡɴᴇʀ @SATYAM_HK"
        else:
            user_role = "👑 ᴏᴡɴᴇʀ"
...
async def attack_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not can_user_attack(user_id):
        await update.message.reply_text(
            ...
            "ᴜsᴇ /start ᴛᴏ ʀᴇǫᴜᴇsᴛ ᴀᴄᴄᴇss.@SATYAM_HK"
        )
        return
...
async def setmaxattack_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if max_attacks < 1 or max_attacks > 1000:
            await update.message.reply_text("❌ ᴍᴀxɪᴍᴜᴍ ᴀᴛᴛᴀᴄᴋs ᴍᴜsᴛ ʙᴇ ʙᴇᴛᴡᴇᴇɴ 1 ᴀɴᴅ 1000")
            return
